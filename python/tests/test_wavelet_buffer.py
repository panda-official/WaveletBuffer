...
from wavelet_buffer import WaveletBuffer, WaveletType, distance, energy_distribution  # pylint: disable=no-name-in-module
...
mark_2d_dimension = pytest.mark.parametrize(
    'params', [parameters_2d()]
)

mark_1d_dimension = pytest.mark.parametrize(
...
def test__none_decomposition_for_small_signals():
    ...
    """
    buffer = WaveletBuffer(signal_shape=[2, 2],
                           signal_number=1,
...
def test__energy_distribution_1d(buffer):
    ...
    """
    assert np.all(
        energy_distribution(buffer) - np.array(
            [1250., 2313.1018, 324786.94]) < 0.01)
...
def test__energy_distribution_2d(buffer):
    ...
    assert np.all(
        energy_distribution(buffer) - np.array(
            [[1.9237234e-10, 4.0000000e+03, 2.2737368e-12, 3.5494452e-10, 7.4019268e+03,
              5.7002656e-11, 3.9939819e+05]
             ] * 3) < 0.01)
